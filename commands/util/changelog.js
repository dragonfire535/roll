...
const { GITHUB_USERNAME, GITHUB_PASSWORD, ROLL_GITHUB_REPO_USERNAME, ROLL_GITHUB_REPO_NAME } = process.env;
...
module.exports = class ChangelogCommand extends Command {
	...
	async run(msg) {
		const { body } = await request
			.get(`https://api.github.com/repos/${ROLL_GITHUB_REPO_USERNAME}/${ROLL_GITHUB_REPO_NAME}/commits`)
			.set({ Authorization: `Basic ${base64(`${GITHUB_USERNAME}:${GITHUB_PASSWORD}`)}` });
		const commits = body.slice(0, 10);
		const embed = new MessageEmbed()
			.setTitle(`[${ROLL_GITHUB_REPO_NAME}:master] Latest 10 commits`)
			.setColor(0x7289DA)
			.setURL(`https://github.com/${ROLL_GITHUB_REPO_USERNAME}/${ROLL_GITHUB_REPO_NAME}/commits/master`)
